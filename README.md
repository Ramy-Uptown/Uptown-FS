...
- [2025-10-18 00:40] Target-PV modes (2 and 4) now correctly account for Down Payment as percentage or amount: Enabled DP Type = percentage in the client for target-PV modes and updated backend solver to include DP% in both PV and nominal equations. This removes the need to set DP to 0 to hit the target PV. Files: api/services/calculationService.js (algebraic solver for DP%), client/src/components/calculator/InputsForm.jsx (allow DP% selection in target-PV modes).
