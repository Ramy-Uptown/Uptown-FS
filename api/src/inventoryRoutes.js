  ...
  try {
    ...
    const r = await pool.query(
      ...
      [
        ...
        !!has_garden,
        has_garden ? (Number(garden_area) || 0) : null,
        !!has_roof,
        has_roof ? (Number(roof_area) || 0) : null,
        garage_area == null ? null : (Number(garage_area) || 0),
        garage_standard_code || null,
        req.user.id
      ]
    )
    const item = r.rows[0]
    await pool.query(
      ...
      [item.id, req.user.id, JSON.stringify({ after: item })]
    )
    return ok(res, { item })
  } catch (e) {
...
router.patch('/unit-models/:id', authMiddleware, requireRole(['financial_manager']), async (req, res) => {
  try {
    ...
    const cur = await pool.query('SELECT * FROM unit_models WHERE id=$1', [id])
    ...
    for (const k of allow) {
      if (req.body.hasOwnProperty(k)) {
        ...
        if (k === 'garden_area' && !req.body.hasOwnProperty('has_garden')) {
          // keep prev has_garden when only area provided
        }
        updatedPreview[k] = v
        params.push(v)
        fields.push(`${k}=${'
...
router.delete('/unit-models/:id', authMiddleware, requireRole(['financial_manager']), async (req, res) => {
  ...
  try {
    ...
    const prev = cur.rows[0]
    await client.query('DELETE FROM unit_models WHERE id=$1', [id])
    await client.query(
      `INSERT INTO unit_model_audit (model_id, action, changed_by, details)
       VALUES ($1, 'delete', $2, $3::jsonb)`,
      [id, req.user.id, JSON.stringify({ before: prev })]
    )
    await client.query('COMMIT'); client.release()
